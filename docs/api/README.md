...
The installer creates missing pipeline and document files, renders the managed
block, and wires all skills. Existing project documents are preserved.
...
node skills/kg-compile/scripts/render-agents.mjs [--check]
...
`scripts/lib/protocol.mjs` exports protocol loaders, schema validation,
frontmatter parsing, and rendering helpers. `scripts/lib/host.mjs` exports
host-root discovery, paths, config loading, ID allocation, and compile-round
action logging. `scripts/lib/agents-block.mjs` exports managed-block rendering
and application.
